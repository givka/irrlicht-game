...
WaveGenerator::WaveGenerator(int health_increment, int health_variance, int damage_increment, int damage_variance, int wave_per_new_enemy, int nb_enemy_variance) : m_health_increment(health_increment), m_health_variance(health_variance),
...
void WaveGenerator::spawnWave(Level &level, int wave_id, Computer &computer, irr::IrrlichtDevice *device, is::ITriangleSelector *selector)
{
    ...
    for (int sp : generated)
    {
        ...
        spawn.scale = (float)(1.0 + wave_id / 10.0f);
        (float)(1.0 + rand() / 4.0);
